...
void GetObjectByStream(qcloud_cos::CosAPI& cos, const std::string& bucket_name,
  ...
  req.AddHeader("Range", "bytes=150-300");
...
void DescribeMediaBuckets(qcloud_cos::CosAPI& cos) {
  ...
  // req.SetBucketNames("xxx");
  // 设置存储桶名称前缀，前缀搜索
  req.SetBucketName("etest-1253127739");
  // 设置第几页
  req.SetPageNumber("1");
  // 设置每页个数
  req.SetPageSize("1");
...
main(int argc, char** argv) {
  ...
  qcloud_cos::CosConfig config(0, "", "", "");
  // config.SetLogCallback(&TestLogCallback);
  qcloud_cos::CosAPI cos(config);
  std::string bucket_name =
      "";  //替换为用户的存储桶名，由bucketname-appid
  std::cout << config.GetAppId() << std::endl;
  // DownloadFromDirectory(cos, bucket_name, "stream657619/", "/tmp/");
  // GetObjectByFile(cos, bucket_name, "db1_db2_1.sql",
  // "/data/dev/cos-cpp-sdk-v5-5.5.8/build/db1_db2_1.sql");
  GetObjectByStream(cos, bucket_name, "db1_db2_1.sql");
  // BaseReq::SetCheckMD5(false);
