...
/* Data. */
JEMALLOC_EXPORT thread_local int64_t last_global_mem_cnt = 0;
JEMALLOC_EXPORT thread_local int64_t je_totalsize = 0;
JEMALLOC_EXPORT thread_local int64_t last_global_mem_snapshot = 0;
bool padding1[64];
JEMALLOC_EXPORT _Atomic(int64_t) je_server_totalsize;
bool padding2[64];
bool config_mon = true;
unsigned long config_chunk_size = 8192L;
...
imalloc_body(static_opts_t *sopts, dynamic_opts_t *dopts, tsd_t *tsd) {
	...
	if (dopts->alignment == 0) {
		...
		if (config_stats || (config_prof && opt_prof) || sopts->usize || config_mon) {
	...
	if (config_mon) {
        inc_thr_and_global_memory(usize, false);
    }
...
je_malloc(size_t size) {
	...
	size_t usize;
	if (config_stats || config_prof || config_mon) {
		usize = sz_index2size(ind);
	}
...
ifree(tsd_t *tsd, void *ptr, tcache_t *tcache, bool slow_path) {
	...
	if (config_mon) {
		usize = sz_index2size(alloc_ctx.szind);
    	dec_thr_and_global_memory(usize, false);
	}
...
je_realloc(void *ptr, size_t arg_size) {
	...
	if (config_mon && likely(ret != NULL)) {
        inc_thr_and_global_memory(usize, false);
		dec_thr_and_global_memory(old_usize, false);
	}
